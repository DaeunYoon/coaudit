...
export default function getFullPathOfImportedFile(
  baseFilePath: string,
  importedFilePath: string,
  compilerRemappings?: Array<string>
) {
    ...
    if (compilerRemappings && compilerRemappings.length > 0) {
      ...
      const { prefix, target } =
        remappings.find(({ context, prefix }) => {
          if (context) {
            const contextPart =
              "/" + stripLeadingSlash(stripTrailingSlash(context));
            return (
              baseFilePath.startsWith(contextPart + "/") &&
              importedFilePath.startsWith(prefix)
            );
          }
