...
export default function addFileImportDecorations(
  ...
  const regularImportDecorations: Array<monaco.editor.IModelDeltaDecoration> =
    regularImportMatches.map((match) => {
      return {
        range: {
          ...match.range,
          startColumn: match.range.startColumn + 8,
  ...
  const namedImportDecorations: Array<monaco.editor.IModelDeltaDecoration> =
    namedImportMatches.map(({ range, matches }) => ({
      range: {
        ...
        startColumn:
          range.startColumn +
          (Array.isArray(matches) ? matches?.[1]?.length + 1 : 0),
  ...
  model.deltaDecorations(
    [],
    regularImportDecorations.concat(namedImportDecorations)
  );
}
